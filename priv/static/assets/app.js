...
`);
  ...
  var Rooms = {
    currentChannel: null,
    // Variável para rastrear o canal atual
    socket: null,
    // Variável para rastrear o soquete
    eventsAdded: false,
    // Controle para adicionar eventos apenas uma vez
    player: null,
    game: null,
    players: null,
    guesses: null,
    ...
    init: function(socket2) {
      console.log("init room");
      this.socket = socket2;
      this.currentChannel = this.socket.channel(`room:lobby`);
      this.currentChannel.join().receive("ok", (resp) => {
    ...
    createRoom: function(roomPW, roomRounds, roomMaxPlayers, roomLanguage, nickname, photoId) {
      ...
      if (!roomRounds || isNaN(roomRounds) || Number(roomRounds) <= 0)
        return alert("Rounds must be a valid number greater than 0");
      if (!roomMaxPlayers || isNaN(roomMaxPlayers) || Number(roomMaxPlayers) <= 0)
        return alert("Max players must be a valid number greater than 0");
      if (!roomLanguage)
        return alert("Language cannot be empty");
      if (!nickname)
        return alert("Nickname cannot be empty");
      if (!photoId)
        return alert("Photo ID cannot be empty");
      if (this.currentChannel) {
        return new Promise((resolve, reject) => {
    ...
    enterRoom: function(roomCode, roomPW, nickname, photoId) {
      if (!roomCode)
        return alert("Room code cannot be empty");
      if (!roomPW)
        return alert("Password cannot be empty");
      if (!nickname)
        return alert("Nickname cannot be empty");
    ...
    getOut: function() {
      ...
      this.currentChannel.push("disconnect", {}).receive("ok", () => console.log("Disconnected from room")).receive("error", () => console.log("Failed to disconnect from room"));
      this.removeListeners();
      this.currentChannel.leave().receive("ok", () => console.log("Left room")).receive("error", () => console.log("Failed to leave room"));
    ...
    sendMusicSelection: function(artist, songName, music_id) {
      if (this.currentChannel) {
        this.currentChannel.push("music_selection", { artist, song_name: songName, music_id });
      }
    },
    ...
    setListenMusicGuesses: function(func) {
      if (this.currentChannel) {
  ...
  var room_default = Rooms;

  // js/app.js
  window.Rooms = room_default;
  var Hooks2 = {};
  Hooks2.LoadSpecificJs = {
    mounted() {
  ...
  window.Rooms.init(user_socket_default);
})();
/**
 * @license MIT
